...
def gather_data(simulation_settings, scripts_folders, num_field_steps_resolution, num_quasistatic_steps_resolution):
    ...
    B_ext_sweep_value_folders = os.listdir(m_dynamics_folder)
    for B_ext_sweep_value_index in range(0, len(B_ext_sweep_value_folders), num_field_steps_resolution):
        B_ext_sweep_value_folder = B_ext_sweep_value_folders[B_ext_sweep_value_index]
        ...
        m_final_quasi_static_steps_folders = os.listdir(m_dynamics_quasi_static_steps_B_ext_sweep_value_folder)
        for quasi_static_step_index in range(0, len(m_final_quasi_static_steps_folders), num_quasistatic_steps_resolution):
            m_final_quasi_static_step = m_final_quasi_static_steps_folders[quasi_static_step_index]
    ...
    B_ext_sweep_value_folders = os.listdir(j_tunnel_folder)
    for B_ext_sweep_value_index in range(0, len(B_ext_sweep_value_folders), num_field_steps_resolution):
        B_ext_sweep_value_folder = B_ext_sweep_value_folders[B_ext_sweep_value_index]
        # Looping through B_ext sweep step folder and appending to m_final_quasi_static_steps
        ...
        j_tunnel_quasi_static_step_folders = os.listdir(j_tunnel_quasi_static_steps_B_ext_sweep_value_folder)
        for quasi_static_step_index in range(0, len(j_tunnel_quasi_static_step_folders), num_quasistatic_steps_resolution):
            j_tunnel_quasi_static_step = j_tunnel_quasi_static_step_folders[quasi_static_step_index]
...
""" PLOTTING MAGNETIZATION DYNAMICS IN ALL CELLS """
def plot_magnetization_configuration_dynamics(B_ext_sweep_step_m_dynamics, simulation_settings):
    Nx = simulation_settings['Nx']
    Ny = simulation_settings['Ny']
    Nz = simulation_settings['Nz']
    fig = plt.figure()
        ...
        for m_final_quasi_static_step in m_dynamics_B_ext_sweep_step:
            ...
            for i in range(Nx):
                for j in range(Ny):
                    ...
                    mx, my, mz = m_cell[0], m_cell[1], m_cell[2]
                    x_vector = [i, i + mx]
                    y_vector = [j, j + my]
                    z_vector = [0, mz]
                    ax.plot(x_vector, y_vector, z_vector, color='r')
            plt.pause(0.1)
        plt.show()
...
def plot_average_magnetization_on_unit_sphere(B_ext_sweep_step_m_dynamics, simulation_settings):
    m_start = simulation_settings['m_free_start_uniform'] 
    for B_ext_sweep_step in range(len(B_ext_sweep_step_m_dynamics)):
        m_dynamics_B_ext_sweep_step = B_ext_sweep_step_m_dynamics[B_ext_sweep_step]
        fig = plt.figure()
        ax = fig.add_subplot(111, projection='3d')
        trajectory = [[m_start[0], m_start[1], m_start[2]]]
        for m_final_quasi_static_step in m_dynamics_B_ext_sweep_step:
            m_x_total = 0
            m_y_total = 0
            m_z_total = 0
        ...
        ax.scatter(m_start[0], m_start[1], m_start[2], s=20, edgecolor='k', linewidth=0.5, zorder=5)
...
def plot_tunnel_current_quasi_static_convergence(B_ext_sweep_step_j_tunnel, simulation_settings, num_quasistatic_steps_resolution):
    ...
    for B_ext_sweep_step in range(len(B_ext_sweep_step_j_tunnel)):
        ...
        plt.figure()
        plt.plot(np.arange(len(total_current_B_ext_sweep_step)) * num_quasistatic_steps_resolution, total_current_B_ext_sweep_step, marker='o', color='tab:red')
...
""" PLOT FINAL VALUE OF TOTAL TUNNEL CURRENT AT EACH QUASI-STATIC STEP """
def plot_j_tunnel_converged(B_ext_sweep_step_j_tunnel, simulation_settings, num_field_steps_resolution):
    size_x = simulation_settings['size_x']
    size_y = simulation_settings['size_y']
    ...
    for B_ext_sweep_step in range(len(B_ext_sweep_step_j_tunnel)):
        ...
        B_labels.append(B_ext_uniform[B_ext_sweep_step * num_field_steps_resolution])
...
""" PLOT TOTAL MTJ RESISTANCE AT EACH QUASI-STATIC STEP """
def plot_R_MTJ_converged(j_tunnel_converged, simulation_settings, num_field_steps_resolution):
    size_x = simulation_settings['size_x']
    size_y = simulation_settings['size_y']
    B_ext_uniform = simulation_settings['B_ext_uniform']
    ...
    for i, j_converged in enumerate(j_tunnel_converged):
        ...
        B = B_ext_uniform[i * num_field_steps_resolution]
...
def plot_results(simulation_settings, scripts_folders, plot_options):
    ...
    B_ext_sweep_step_m_dynamics, B_ext_sweep_step_j_tunnel = gather_data(
        simulation_settings,
        scripts_folders,
        plot_options['num_field_steps_resolution'],
        plot_options['num_quasistatic_steps_resolution']
    )

    # Plotting magnetization configuration evolution at each B_exy sweep step
    if plot_options['show_configuration_dynamics']:
        plot_magnetization_configuration_dynamics(B_ext_sweep_step_m_dynamics, simulation_settings)
    ...
    if plot_options['show_j_tunnel_convergence']:
        plot_tunnel_current_quasi_static_convergence(B_ext_sweep_step_j_tunnel, simulation_settings, plot_options['num_quasistatic_steps_resolution'])

    # Final converged tunnel current at each B_ext sweep step
    if plot_options['show_j_tunnel_final']:
        plot_j_tunnel_converged(B_ext_sweep_step_j_tunnel, simulation_settings, plot_options['num_field_steps_resolution'])

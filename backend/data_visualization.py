...
def plot_average_magnetization_on_unit_sphere(m_dynamics_full):
    ...
    colors = plt.cm.get_cmap('tab10', len(m_dynamics_full))
    for j_sim_iter, simulation_steps in enumerate(m_dynamics_full):
        trajectory = []
    ...
    ax.set_ylabel('my')
    ax.set_zlabel('mz')
    ax.set_title("Normalized Average Magnetization Trajectory on Unit Sphere")
...
def plot_average_tunnel_current(j_tunnel_full, simulation_settings):
    ...
    avg_currents = []
    for j_iter_data in j_tunnel_full:
    ...
    plt.title('Convergence of Total Tunnel Current')
    plt.grid(True)
    plt.tight_layout()
    plt.show()
...
def plot_results(simulation_settings, scripts_folders, plot_options):
    ...
    if plot_options['show_unit_sphere_dynamics']:
        # Loop through simulation iteration folders and append to m_dynamics_full in such a way that
        # the ith LLGS step of the jth simulation iteration can be accesed as: m_dynamics_full[j][i]
        for m_dynamics_iteration_folder in os.listdir(m_dynamics_folder):
            iteration_path = os.path.join(m_dynamics_folder, m_dynamics_iteration_folder)
            m_dynamics_simulation_steps = []
            ...
            for ovf_filename in os.listdir(iteration_path):
                ...
                m_dynamics_simulation_steps.append(m_data)
            # Appending simulation iteration
            m_dynamics_full.append(m_dynamics_simulation_steps)
    ...
    if plot_options['show_j_tunnel_convergence']:
        # Loop through simulation iteration folders and append to j_tunnel_full in such a way that
        # the jth simulation iteration can be accessed as: j_tunnel_full[j]
        for j_tunnel_iteration_folder in os.listdir(j_tunnel_folder):
            iteration_path = os.path.join(j_tunnel_folder, j_tunnel_iteration_folder)
            ovf_filename = os.listdir(iteration_path)[0]
            ovf_path = os.path.join(iteration_path, ovf_filename)
            j_tunnel_data = extract_data(ovf_path, simulation_settings)
            # Appending simulation iteration result
            j_tunnel_full.append(j_tunnel_data)

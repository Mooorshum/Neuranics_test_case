...
def get_B_ext_data(simulation_settings):
    Nx = simulation_settings['Nx']
    Ny = simulation_settings['Ny']
...
def run_simulation(simulation_settings, scripts_folders):
    ...
    B_ext_data = get_B_ext_data(simulation_settings)
    convert_to_ovf(
        'B_ext_data.ovf',
    ...
    for num_iter in range(num_iterations):
        ...
        # Running MTJ script
        subprocess.run(['mumax3', scripts_folders['MTJ_SCRIPT_INSTANCE']])
        ...
        m_dynamics_iter_folder = f'{scripts_folders["M_DYNAMICS_DATA_FOLDER"]}/m_dynamics_iteration_{num_iter}'
        ...
        j_tunnel_iter_folder = f'{scripts_folders["J_TUNNEL_DATA_FOLDER"]}/j_tunnel_iteration_{num_iter}'
        ...
        print(f'****** STEP {num_iter + 1} / {num_iterations} COMPUTED ******')
